...
JVM_ARGS="-XX:MaxDirectMemorySize=256m -XX:+UseStringDeduplication -XX:+UseG1GC -Xlog:gc*,safepoint=info:./logs/gc.log:time,uptime,level,tags:filecount=5,filesize=100M -XX:+HeapDumpOnOutOfMemoryError -XX:+ExitOnOutOfMemoryError -XX:HeapDumpPath=${LOG_PATH}/dump -XX:ErrorFile=${LOG_PATH}/hs_err_%p.log"
#优雅关机等待时间
SHUTDOWN_SECONDS=30
...
# 服务名称
export OTEL_SERVICE_NAME=app-server
# 资源属性（可选）
export OTEL_RESOURCE_ATTRIBUTES="service.name=${OTEL_SERVICE_NAME},service.version=1.0.0,deployment.environment=dev"
# 认证 Token
